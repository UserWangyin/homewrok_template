...
int KnapSackDP(int n,int weight[],int value[],int C);  //n为物品个数，weight为每个物品的重量，value为每个物品的价值，C为背包承受的最大重量
void Judge(int C,int n,int weight[]);  //判断背包是否可以承受
void RandomLGCW(int *list,int n,int l,int r);   //生成范围在l~r的随机物品重量 
void RandomLGCV(int *list,int n,int l,int r);  //生成范围在l~r的随机物品价值
void initpackage();
...
void RandomLGCV(int *list,int n,int l,int r)   //生成范围在l~r的随机随机物品价值
{
    int seed=1;
	srand(seed);                     //设置时间种子
	for(int i=0;i<n;i++){
